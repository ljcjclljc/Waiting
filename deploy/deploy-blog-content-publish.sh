...
die() { printf 'Content deployment failed: %s\n' "$*" >&2; exit 1; }
[[ $# -eq 1 ]] || die 'expected one content digest'
readonly DIGEST="$1"
[[ "${DIGEST}" =~ ^[0-9a-f]{40}$ ]] || die 'invalid content digest'
for command_name in basename chmod curl docker du find flock grep head install mktemp mv rm rsync sha256sum sleep tar tr wc; do
...
[[ "${INCOMING_SIZE}" -gt 0 && "${INCOMING_SIZE}" -le "${MAX_BYTES}" ]] || die 'invalid content bundle size'
readonly ACTUAL_DIGEST="$(sha256sum "${INCOMING_TMP}" | cut -c1-40)"
[[ "${ACTUAL_DIGEST}" == "${DIGEST}" ]] || die "content digest mismatch: expected ${DIGEST}, got ${ACTUAL_DIGEST}"
...
if tar -tf "${INCOMING_FILE}" | grep -E '(^/|^[A-Za-z]:/|(^|/)\.\.(\/|$))' >/dev/null; then die 'content bundle contains unsafe paths'; fi
if tar -tvf "${INCOMING_FILE}" | grep -Eq '^l'; then die 'content bundle contains symlinks'; fi
...
find "${STAGED_POSTS}" -type l -print -quit | grep -q . && die 'content symlinks are not allowed' || true
file_count="$(find "${STAGED_POSTS}" -type f | wc -l)"
byte_count="$(du -sb "${STAGED_POSTS}" | cut -f1)"
[[ "${file_count}" -le "${MAX_FILES}" && "${byte_count}" -le "${MAX_BYTES}" ]] || die 'content bundle exceeds limits'
...
for attempt in $(seq 1 30); do
    ...
    if grep -Eq '"contentReload"[[:space:]]*:[[:space:]]*"up"' <<<"${health}" && grep -Eq "\"contentVersion\"[[:space:]]*:[[:space:]]*\"${DIGEST}\"" <<<"${health}"; then
